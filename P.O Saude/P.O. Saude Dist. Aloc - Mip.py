...
class Set_Covering:
    ...
    def model(self):
        ...
        # Create the model
        model = Model("Set-covering", sense=MINIMIZE)
        model.verbose = 0
        # Variable
        x = {j: model.add_var(name=f"x[{j}]", var_type=BINARY) for j in J}
        # Objective function
        model.objective = xsum(f[j] * x[j] for j in J)

        # Restrictions
        for i in I:
            model += xsum(x[j] for j in N[i]) >= 1
    ...
    def print_solution(self):
        ...
        print(f"Selected Locations: {sum(model.var_by_name(f'x[{j}]').x for j in J)}")
        print("===========================================")
        for i in I:
            for j in N[i]:
                if model.var_by_name(f'x[{j}]').x > 0.5:
...
class Maximal_Covering:
    ...
    def model(self):
        ...
        model = Model("Maximal covering", sense=MAXIMIZE)
        model.verbose = 0

        x = {j: model.add_var(name=f"x[{j}]", var_type=BINARY) for j in J}
        z = {i: model.add_var(name=f"z[{i}]", var_type=BINARY) for i in I}

        model.objective = xsum(w[i]*z[i] for i in I)

        model += xsum(x[j] for j in J) == p
        for i in I:
            model += xsum(x[j] for j in N[i]) >= z[i]
        # Resolução do modelo
        self._model = model
        model.optimize()
    ...
    def print_solution(self):
        ...
        print(f"Total Covered Demand: {model.objective_value:.2f}\n")
        print(f"Selected Locations: {sum(model.var_by_name(f'x[{j}]').x for j in J)}")
        print("===========================================")
        for i in I:
            for j in N[i]:
...
class P_Center:
    ...
    def model(self):
        ...
        model = Model("P-center", sense=MINIMIZE)
        model.verbose = 0

        L = model.add_var(name="L")

        x = {j: model.add_var(name=f"x[{j}]", var_type=BINARY) for j in J}
        y = {(i, j): model.add_var(name=f"y[{i},{j}]", var_type=BINARY) for i in I for j in N[i]}

        model.objective = L

        # Constraint (10): Each demand point i must be assigned to exactly one facility j
        for i in I:
            model += xsum(y[i,j] for j in N[i]) == 1
        ...
        for i in I:
            model += xsum(w[i] * d[i,j] * y[i,j] for j in N[i]) <= L
    ...
    def print_solution(self):
        ...
        print("===========================================")
        print("P Center")
        print(f"Maximum Distance: {model.objective_value:.2f}\n")
        print(f"Selected Locations: {sum(model.var_by_name(f'x[{j}]').x for j in J)}")
        print("===========================================")
        for i in I:
            for j in N[i]:
                if model.var_by_name(f'y[{i},{j}]').x > 0.5:
...
class P_Median:
    ...
    def model(self):
        ...
        model = Model("P-median", sense=MINIMIZE)
        model.verbose = 0

        x = {j: model.add_var(name=f"x[{j}]", var_type=BINARY) for j in J}
        y = {(i,j):model.add_var(name=f"y[{i},{j}]", var_type=BINARY) for j in J for i in I}

        # Objective function: minimize the weighted travel time
        model.objective = xsum(w[i] * d[i,j] * y[i,j] for i in I for j in J)
    ...
    def print_solution(self):
        ...
        print("===========================================")
        print("P Median")
        print(f"total demand-weighted distance: {model.objective_value:.2f}")
        print("\nSelected Locations:")
        for j in J:
            if model.var_by_name(f"x[{j}]").x >= 0.99:  # Verifica se a instalação foi selecionada
                print(f"Facility {j} is selected")
        print("===========================================")
        for i in I:
            for j in J:
                if model.var_by_name(f"y[{i},{j}]").x > 0.5:  # Verifica se a demanda foi atribuída à instalação
                    print(f"[{i}]\tis assigned to [{j}]: {d[i,j]:.2f} km")
        print("===========================================")
...
class Fixed_Charge:
    ...
    def model(self):
        ...
        model = Model("Fixed-charge", sense=MINIMIZE)
        model.verbose = 0
        ...
        model.objective = xsum(f[j] * x[j] for j in J) + xsum(v * w[i] * d[i,j] * y[i,j] for i in I for j in J)
        ...
        for j in J:
            model += xsum(w[i] * y[i,j] for i in I) <= U[j]
        self._model = model
        model.optimize()
    def print_solution(self):
        model = self._model
        ...
        print("===========================================")
        print("Fixed Charge")
        print(f"total fixed and variable costs: {model.objective_value:.2f}\n")
        print(f"Selected Locations: {sum(model.var_by_name(f'x[{j}]').x for j in J)}")
        print("===========================================")
        for i in I:
            for j in J:
...
class Hierarchical:
    ...
    def model(self):
        ...
        model = Model("Hierarchical", sense=MINIMIZE)
        model.verbose = 0

        x1 = {k:    model.add_var(name=f"x1[{k}]", var_type=BINARY) for k in K}
        x2 = {j:    model.add_var(name=f"x2[{j}]", var_type=BINARY) for j in J}
        u  = {(i,k):model.add_var(name=f"u[{i},{k}]") for i in I for k in K }
        v  = {(k,j):model.add_var(name=f"v[{k},{j}]") for k in K for j in J }

        # Objective function: minimize the travel time
        model.objective = xsum(d1ik[i,k] * u[i,k] for i in I for k in K) + xsum(d2kj[k,j] * v[k,j] for k in K for j in J)

        # Constraint (29): Each demand point i must be assigned to exactly one level-1 PCF k
        for i in I:
            model += xsum(u[i,k] for k in K) == w[i]

        # Constraint (30): The flow of patients from level-1 PCF k to level-2 PCF j
        for k in K:
            model += xsum(v[k,j] for j in J) == O[k] * xsum(u[i,k] for i in I)

        # Constraint (31): The total demand assigned to each level-1 PCF k must not exceed its capacity
        for k in K:
            model += xsum(u[i,k] for i in I) <= C1[k] * x1[k]

        # Constraint (32): The total demand assigned to each level-2 PCF j must not exceed its capacity
        for j in J:
            model += xsum(v[k,j] for k in K) <= C2[j] * x2[j]

        # Constraint (33): Exactly p level-1 PCF locations must be established
        model += xsum(x1[k] for k in K) == p

        # Constraint (34): Exactly q level-2 PCF locations must be established
        model += xsum(x2[j] for j in J) == q
    ...
    def print_solution(self):
        ...
        if model.num_solutions:
            ...
            print("Hierarchical")
            print(f"Total Cost: {model.objective_value:.2f}\n")
            print(f"Selected Level-1 PCF Locations: {sum(model.var_by_name(f'x1[{k}]').x for k in K)}")
            print(f"Selected Level-2 PCF Locations: {sum(model.var_by_name(f'x2[{j}]').x for j in J)}")
            print("===========================================")
            for k in K:
                if model.var_by_name(f'x1[{k}]').x > 0.5:
                    print(f"Level-1 PCF at \t{k}\t covers:")
                    covered_demands = [i for i in I if model.var_by_name(f'u[{i},{k}]').x > 0.5]
                    print(f"  Demand points covered: {', '.join(covered_demands)}")
            print("===========================================")
            for j in J:
                if model.var_by_name(f'x2[{j}]').x > 0.5:
...
if __name__ == "__main__":
    ...
    K = ["D",'E','F','G','H'] # K level-1 PCF and secondary 
