from pulp import LpProblem, LpMinimize, lpSum, LpVariable, PULP_CBC_CMD,LpBinary
import time
...
class Hierarchical:
    ...
    def Model(self):
        ...
        y  = {(i,j): LpVariable(f"y_{i}_{j}", cat=LpBinary) for i in I for j in L[1]}
        y1 = {j: LpVariable(f"y1_{j}", cat=LpBinary) for j in L[1]}
        y2 = {j: LpVariable(f"y2_{j}", cat=LpBinary) for j in L[2]}
        y3 = {j: LpVariable(f"y3_{j}", cat=LpBinary) for j in L[3]}
        
        u1 = {(p, i, j):LpVariable(f"u1_{p}_{i}_{j}", lowBound=0)  for p in P for i in I for j in L[1]}
        u2 = {(j, j2):  LpVariable(f"u2_{j}_{j2}", lowBound=0)  for j in L[1] for j2 in L[2]}
        u3 = {(j2, j3): LpVariable(f"u3_{j2}_{j3}", lowBound=0) for j2 in L[2] for j3 in L[3]}

        l1 = {(e, l):  LpVariable(f"l1_{e}_{l}")  for e in E[1] for l in L[1]}
        l2 = {(e, l):  LpVariable(f"l2_{e}_{l}")  for e in E[2] for l in L[2]}
        l3 = {(e, l):  LpVariable(f"l2_{e}_{l}")  for e in E[3] for l in L[3]}
        start = time.time()
